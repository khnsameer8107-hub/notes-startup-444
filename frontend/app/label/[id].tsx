  ...
  return (
    <View style={{ flex: 1, backgroundColor: c.surface, paddingTop: insets.top }}>
      <View style={styles.header}>
        <Pressable testID="label-view-back" onPress={() => router.back()} style={styles.hBtn}>
          <MaterialCommunityIcons name="arrow-left" size={24} color={c.onSurface} />
        </Pressable>
        <Text style={[styles.hTitle, { color: c.onSurface }]} numberOfLines={1}>#{name || "Label"}</Text>
        <View style={{ width: 40 }} />
      </View>

      {notes.length === 0 ? (
        <EmptyState icon="tag-outline" title="No notes with this label" subtitle="Add this label to notes to see them here." testID="empty-label-view" />
      ) : (
